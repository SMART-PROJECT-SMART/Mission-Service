...
namespace Mission_Service.Controllers
{
  ...
  private readonly ILogger<AssignmentController> _logger;
        ...
        public AssignmentController(IAssignmentAlgorithm assignmentAlgorithm, ILogger<AssignmentController> logger)
 {
...
_logger.LogInformation("========================================");
  ...
  var results = new List<object>();
            ...
            results.Add(ExecuteTestCaseInternal("Test 1: Basic Equal Assignment", CreateBasicEqualAssignmentData()));
            results.Add(ExecuteTestCaseInternal("Test 2: Resource Constrained", CreateResourceConstrainedData()));
            results.Add(ExecuteTestCaseInternal("Test 3: Time Overlap Conflict", CreateTimeOverlapConflictData()));
          results.Add(ExecuteTestCaseInternal("Test 4: Type Mismatch", CreateTypeMismatchData()));
     results.Add(ExecuteTestCaseInternal("Test 5: Priority Assignment", CreatePriorityAssignmentData()));
      results.Add(ExecuteTestCaseInternal("Test 6: Telemetry Optimization", CreateTelemetryOptimizationData()));
            results.Add(ExecuteTestCaseInternal("Test 7: Mixed Types", CreateMixedTypesData()));
    results.Add(ExecuteTestCaseInternal("Test 8: Sequential Missions", CreateSequentialMissionsData()));
        ...
        public IActionResult TestStressTest()
{
 ...
 private IActionResult ExecuteTestCase(string testName, List<Mission> missions, List<UAV> uavs)
        {
          var result = ExecuteTestCaseInternal(testName, (missions, uavs));
     return Ok(result);
     }

        private object ExecuteTestCaseInternal(string testName, (List<Mission> missions, List<UAV> uavs) testData)
        {
  try
            {
      ...
      {
              _logger.LogInformation($"  ID: {mission.Id}, Type: {mission.RequiredUAVType}, Priority: {mission.Priority}, " +
 ...
 _logger.LogInformation($"  TailID: {uav.TailId}, Type: {uav.UavType}");
   ...
   _logger.LogInformation("EXECUTING ALGORITHM...");
     ...
     foreach (var chromosome in result.Assignments)
       {
    _logger.LogInformation($"CHROMOSOME - Fitness: {chromosome.FitnessScore:F6}, Valid: {chromosome.IsValid}, Assignments: {chromosome.Assignments.Count()}");

        foreach (var gene in chromosome.Assignments)
        {
     _logger.LogInformation($"  Mission: {gene.Mission.Id} -> UAV: {gene.UAV.TailId} (Type: {gene.UAV.UavType}), " +
