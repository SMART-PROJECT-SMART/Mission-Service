...
namespace Mission_Service.Services.Genetic_Assignment_Algorithm.Main_Algorithm
{
    public class GeneticAssignmentAlgorithm : IAssignmentAlgorithm
    {
        ...
        public AssignmentResult PreformAssignmentAlgorithm(IEnumerable<Mission> missions, IEnumerable<UAV> uavs)
        {
            ...
            for (int generation = 0; generation < _algorithmConfiguration.MaxGenerations; generation++)
            {
                population = CreateNextGeneration(population, missionList, uavList);

                EvaluateFitnessParallel(population);

                AssignmentChromosome currentBestChromosome = GetChromosomeWithBestFitnessScore(population);
        ...
        private AssignmentChromosome GetChromosomeWithBestFitnessScore(List<AssignmentChromosome> population)
        {
            return population.OrderByDescending(chromosome => chromosome.FitnessScore).First();
        }

        private bool IsAssignmentBetter(AssignmentChromosome newAssignment, AssignmentChromosome currentBestAssignment)
        {
            ...
            List<UAV> uavs)
        {
            ...
            List<AssignmentChromosome> offspringPopulation = CreateOffspringParallel(currentPopulation, offspringCount, uavs);

            RepairPopulationParallel(offspringPopulation, missions, uavs);

            List<AssignmentChromosome> newPopulation = new List<AssignmentChromosome>(elite.Count + offspringPopulation.Count);
        ...
        private List<AssignmentChromosome> SelectEliteChromosomes(List<AssignmentChromosome> population)
        {
            int eliteCount = (int)(_algorithmConfiguration.PopulationSize * _algorithmConfiguration.ElitePrecentage);
        ...
        private List<AssignmentChromosome> CreateOffspringParallel(
            ...
            List<UAV> uavs)
        {
            int pairCount = (offspringCount + 1) / MissionServiceConstants.MainAlgorithm.OFFSPRING_PAIR_SIZE;
            AssignmentChromosome[] offspringPopulation = new AssignmentChromosome[offspringCount];

            Parallel.For(0, pairCount, pairIndex =>
            {
                AssignmentChromosome parent1 = _selectionStrategy.selectParentChromosome(population);
                AssignmentChromosome parent2 = _selectionStrategy.selectParentChromosome(population);
                ...
                int firstOffspringIndex = pairIndex * MissionServiceConstants.MainAlgorithm.OFFSPRING_PAIR_SIZE +
                                          MissionServiceConstants.MainAlgorithm.FIRST_OFFSPRING_INDEX;
                int secondOffspringIndex = pairIndex * MissionServiceConstants.MainAlgorithm.OFFSPRING_PAIR_SIZE +
                ...
                {
                    offspringPopulation[secondOffspringIndex] = crossoverResult.SecondChromosome;
                }
            });
        ...
        private CrossoverResult ApplyCrossover(AssignmentChromosome parent1, AssignmentChromosome parent2)
        {
            ...
            {
                FirstChromosome = parent1,
                SecondChromosome = parent2
            };
        }
