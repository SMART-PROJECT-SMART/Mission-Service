...
{
    public static T SelectRandom<T>(IReadOnlyList<T> list) => list[Random.Shared.Next(list.Count)];

    public static T SelectRandom<T>(T[] array) => array[Random.Shared.Next(array.Length)];
    ...
    public static int GetRandomIndex(int maxValue) => Random.Shared.Next(maxValue);
    ...
    public static bool ShouldOccur(double probability) => Random.Shared.NextDouble() < probability;
